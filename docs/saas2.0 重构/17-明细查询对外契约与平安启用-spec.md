...
### 0.1 补充裁决（2026-09-08，登记簿明细 24 放开 98/99）

1. 24 的 `AccountDetailType` 对外放开为 `04/98/99`：`98` 全部明细、`99` 全部汇总**仅中信生效**
   （中信协议原生支持、Capability 协议层白名单本就全量）；平安 6073 无「全部」类查询，
   对应值不放开、维持 Capability 内仅接受 04（其余值返回 `CAPABILITY_NOT_SUPPORTED`），
   组装器与平安共用枚举白名单、由 Capability 拒绝。
2. 行 `transType` 回填：04 查询维持 §1.2「统一回填 04」；98/99 查询回填银行原始 `TRANS_TYPE`
   （行类型混合，JJ02/JJ08 等原值直出）。
3. `fee` 手续费语义仅在 04 查询下成立；98/99 行该字段承载银行 `TRANS_AMT`（交易金额，分），
   不做特殊处理（用户裁决：该字段除提现手续费外业务不消费），文档须标注语义边界。
4. 25 平台明细不变，仍为 01/02/03，不放开 99。
