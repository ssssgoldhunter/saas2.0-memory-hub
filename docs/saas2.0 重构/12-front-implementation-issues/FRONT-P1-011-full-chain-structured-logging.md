...
> 注：本节的"已脱敏"表述已由 2026-08-14 用户确认的日志明文策略取代（见文末"后续策略变更"），
> 但"只记录一次、字段层级完整、不含反射和交易型 metadata"等核心验收项不变。
...
## 后续策略变更（2026-08-14 用户确认，不改变本问题验收结论）

- 用户确认取消 Front 日志敏感值掩码：`FrontLogJsonUtils` 不再掩码，钱包前后、API、Handle 日志的
  账号、卡号、姓名、手机号、证件号、验证码等字段值均按明文输出；
- 本问题的验收标准 2、5 中“已脱敏”表述随之失效，但“字段和层级完整、只记录一次、不含反射和
  交易型 metadata”等核心验收项保持不变；`appKey`、签名头、`Authorization` 和完整银行 URL 仍不进入日志。
