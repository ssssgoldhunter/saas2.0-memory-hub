...
> **行为准则**：`docs/开发操作手册.md`（高级工程师行为准则，合并权威版，三份工具手册的公共规则层）。
...
2. **需求/功能完成后、提交前**：更新项目记忆体（MEMORY.md / WIKI-START.md 对应章节）并蒸馏结构/业务
   （蒸馏由 agent 判断是否要做；判断不了先问用户「是否对结构/业务蒸馏」——见 `docs/开发操作手册.md` §4）。
3. 有重要发现时更新记忆体（技术决策、常见问题、高频规则）。
4. **自动 push 记忆库到 GitHub**（有新提交即 `git push origin main`，无需询问）。
5. **代码仓库的 commit / push 必须先经用户确认**，确认后才分别提交代码仓库和记忆库。
...
**文档同步偏好（2026-09-07 用户确认）**：更新设计、开发和业务文档时，只修改对应
`*-memory-hub` 记忆体仓库；不要同步修改代码仓库中的 `docs/`、README 或其他文档副本，
代码仓库侧由用户手动复制。除非用户在当次任务中明确要求，否则不得改写这些副本。
...
- ShardingSphere STANDARD 分片键固定 `tenant_id`（进程内租户映射缓存路由，
  `data_source_id` 列值与路由同源：`TenantBankConfigLoader` 经 `TenantDataSourceMappingCache`
  取得，形如 `ds_N`；`tenant_base_config` 残留值仅 WARN 观测）；`tenant_id` 缺失/映射缺失/
  目标 `ds_x` 不存在必须立即失败，禁止默认进入 `ds_0`。
